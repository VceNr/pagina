...
app.post('/usuarios', async (req, res) => {
    const { nombre, correo } = req.body; // Recibir datos del usuario desde la solicitud POST
    if (!nombre || !correo) {
        return res.status(400).send('Faltan datos requeridos: nombre y correo');
    }

    try {
        const request = new sql.Request();
        const query = `INSERT INTO dbo.usuarios (nombre, correo) VALUES (@nombre, @correo)`;
        
        // Parámetros para evitar inyección SQL
        request.input('nombre', sql.NVarChar, nombre);
        request.input('correo', sql.NVarChar, correo); // Cambié 'email' a 'correo'
